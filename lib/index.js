...
export function apply(ctx, config) {
  ...
  function liveSockets() {
    try {
      return ctx.webServer?.upgradedSockets?.size ?? 0
    } catch {
      return 0
    }
  }
  ...
  function exitLikeCtrlC(forceDry = false) {
    ...
    if (typeof exit === 'function') exit(EXIT_CODE)
  ...
  if (tools) {
    ctx.effect(() => tools.register(defineTool({
      ...
      parameters: {
        action: { type: 'string', required: true, description: 'status | toggle | test' },
      },
      output: {
        schema: { type: 'string' },
      ...
      async execute(args) {
        ...
        if (action === 'toggle') runtimeEnabled = !runtimeEnabled
        if (action === 'test') {
          exitLikeCtrlC(true)
